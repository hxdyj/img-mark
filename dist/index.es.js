import { defineComponent, ref, onBeforeUnmount, onMounted, watch, openBlock, createElementBlock, withModifiers, createElementVNode, normalizeClass, toDisplayString, pushScopeId, popScopeId, nextTick, createTextVNode } from "vue";
...
function fixRectInfo(boundingBox) {
  let newInfo = lodash.exports.cloneDeep(boundingBox);
...
function getTwoRectIntersectPart(rect1, rect2) {
  ...
  if (intersectX !== void 0 && intersectY !== void 0) {
    return Object.assign(intersectY, intersectX);
  }
  return void 0;
}
function transfromBoundingBoxToLtwh(position, scale = 1, zoomScale = 1, currentPosition = {
  x: 0,
  y: 0
}) {
...
function drawTagList(ctx, list, zoomScale, currentPosition, offsetInfo = {
  offsetX: 0,
  offsetY: 0
}, origin = {
  x: 0,
...
function moveDrawCropRect(ctx, startPoint, endPoint, zoomScale = 1, origin) {
  if (startPoint.x !== void 0 && endPoint.x !== void 0) {
...
function moveDrawTagRect(ctx, startPoint, endPoint, zoomScale = 1, origin, tagArr, currentPosition) {
  if (startPoint.x !== void 0 && endPoint.x !== void 0) {
    ...
    if (position[2] > 5 || position[3] > 5) {
      ...
      drawTagRect(ctx, ...position, tagArr.length + 1, void 0, true);
      return position;
    }
  }
  return void 0;
...
function moveDrawUnshowTagDashRect(ctx, mode, tagArr, zoomScale, currentPosition, origin, e, cropInfo, cropScale, isWheeling, hasHoverRectInCanvas) {
  if (mode === "tag" && !isWheeling) {
    ...
    if (isHasTouchPointInArr) {
      ...
      drawTagList(ctx, dashArr, zoomScale, currentPosition, void 0, void 0, touchPoint);
    } else {
      if (hasHoverRectInCanvas) {
        ...
        drawTagList(ctx, tagArr, zoomScale, currentPosition, void 0, void 0);
...
function detectEventIsTriggerOnCropBorderOrVertex(event, cropInfo, cropScale, zoomScale, currentPosition, origin) {
  ...
  let detectResult = pointIsInRectList(touchPoint, borderList.map((i) => i.positions));
  return {
...
function moveDetectCropBorderSetCursor(ele, event, mode, cropInfo, cropScale, zoomScale, currentPosition, origin, isWheeling) {
...
function getResizeCropInfo(cropInfo, offsetInfo, borderOrVertexInfo) {
  if (!borderOrVertexInfo)
    return cropInfo;
  let newCropInfo = lodash.exports.cloneDeep(cropInfo);
...
function moveResizeCrop(ctx, startPoint, endPoint, cropInfo, cropScale, zoomScale, currentPosition, tagArr, mouseDownOnCropBorderOrVertex) {
  if (startPoint && startPoint.x !== void 0 && endPoint && endPoint.x !== void 0) {
    let offsetResult = twoPointsGetOffsetInfo(startPoint, endPoint, zoomScale);
    if (offsetResult.isStartMove) {
      let borderOrVertex = mouseDownOnCropBorderOrVertex;
      let { offsetX, offsetY } = offsetResult.offsetInfo;
      let newCropInfo = getResizeCropInfo(cropInfo, {
      ...
      drawCropRect(ctx, ...position);
      drawTagList(ctx, tagArr, zoomScale, currentPosition, void 0, void 0, void 0);
      return position;
    }
  }
  return void 0;
...
var ImgMark_vue_vue_type_style_index_0_scoped_true_lang = "";
...
const _withScopeId = (n) => (pushScopeId("data-v-7e867318"), n = n(), popScopeId(), n);
const _hoisted_1 = ["onMousedown", "onClick", "onMouseup", "onMousemove", "onMouseout", "onMousewheel", "onTouchmove", "onTouchstart", "onTouchend"];
const _hoisted_2 = { class: "mode-panel" };
const _hoisted_3 = { class: "status" };
const _hoisted_4 = { class: "text" };
const _hoisted_5 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createElementVNode("div", { class: "tip" }, [
  /* @__PURE__ */ createElementVNode("kbd", null, "Ctrl"),
  /* @__PURE__ */ createTextVNode(" + "),
  /* @__PURE__ */ createElementVNode("kbd", null, "B"),
  /* @__PURE__ */ createElementVNode("span", { style: { "font-size": "14px" } }, "\u5207\u6362\u6A21\u5F0F")
], -1));
const _sfc_main = /* @__PURE__ */ defineComponent({
  props: {
    enableDrawCropOutOfImg: { type: Boolean, default: true },
    enableDrawTagOutOfCrop: { type: Boolean, default: true },
    enableDrawTagOutOfImg: { type: Boolean, default: true },
    cropBounding: null,
    tagList: { default: () => Array() },
    mode: { default: "crop" },
    src: null
  },
  emits: ["update:tagList", "tagListChange", "update:mode", "update:cropBounding", "tagsStatusChange", "cropChange"],
  setup(__props, { expose, emit: emits }) {
    const props = __props;
    let spaceKeyDown = false;
    let isWheeling;
    let wheelSetTimeout;
    let mouseDownTime = void 0;
    let mouseUpTime = void 0;
    let zoomIntensity = 0.02;
    let mouseDownOnCropBorderOrVertex;
    let hasHoverRectInCanvas = false;
    function initVar() {
      hasHoverRectInCanvas = false;
      zoomIntensity = 0.02;
      mouseDownOnCropBorderOrVertex = void 0;
    }
    let inited = false;
    let isWheeled = false;
    let ctx = null;
    let ctx2 = null;
    let img;
    let canvasWH = lodash.exports.cloneDeep(defaultWH);
    let imgWH = lodash.exports.cloneDeep(defaultWH);
    let startMousePoint = lodash.exports.cloneDeep(defaultPoint);
    let endMousePoint = lodash.exports.cloneDeep(defaultPoint);
    let twoFingerCenterPoint = {
      x: 0,
      y: 0
    };
    let hypotenuse = 0;
    ...
    let scale = 1;
    let cropInfo;
    let tmpCurrentPosition;
    let cropScale = 1;
    let containerInfo;
    let zoomScale = 1;
    let tmpCropPositionInfo;
    let tmpTagPositionInfo;
    let tagArr = [];
    function initDataVar() {
      ...
      ctx2 = null;
      img = void 0;
      canvasWH = lodash.exports.cloneDeep(defaultWH);
      imgWH = lodash.exports.cloneDeep(defaultWH);
      startMousePoint = lodash.exports.cloneDeep(defaultPoint);
      endMousePoint = lodash.exports.cloneDeep(defaultPoint);
      currentPosition = {
        x: 0,
        y: 0
      };
      ...
      scale = 1;
      cropInfo = void 0;
      tmpCurrentPosition = void 0;
      cropScale = 1;
      containerInfo = void 0;
      zoomScale = 1;
      tmpCropPositionInfo = void 0;
      tmpTagPositionInfo = void 0;
      tagArr = [];
    }
    ...
    async function initComponent(isFirst) {
      ...
      let containerRectInfo = containerRef.value.getBoundingClientRect();
      containerInfo = {
      ...
      if (!ctx || !ctx2)
        return Promise.reject(`Error: can't find canvas element.`);
      ...
      return loadImage(props.src).then((img2) => {
        if (!canvasWH || !ctx || !ctx2)
        ...
        console.log("Image WH", imgWH, canvasWH);
        let initScaleInfo = initScale(canvasWH, img2);
        scale = cropScale = initScaleInfo.scale;
        console.log("Scale", scale);
        console.log("Image Current", currentPosition.x, currentPosition.y, imgWH.width * scale, imgWH.height * scale);
        if (isFirst === true) {
          if (!cropInfo) {
            ...
            console.log("RATE", widthRate, heightRate);
            let boxStretchScale = cropBoxInfo[2] >= cropBoxInfo[3] ? widthRate : heightRate;
            let canvasZoom = Math.sqrt(boxStretchScale);
            console.log("currentPosition before", currentPosition);
        ...
        drawImage(ctx, img2, currentPosition.x, currentPosition.y, img2.width * scale * zoomScale, img2.height * scale * zoomScale);
        let initPosition = transfromBoundingBoxToLtwh(cropInfo, cropScale, zoomScale, currentPosition);
        console.log("Crop Current", initPosition);
    ...
    watch(() => props.tagList, (list) => {
      ...
      drawTagList(ctx2, tagArr, zoomScale, currentPosition, void 0, void 0, void 0);
    });
    function onMouseWheel(e, privateCall) {
      let event = e;
      if (!ctx || !ctx2 || !containerInfo || !img || !cropInfo)
        return new Error(`can't find canvas ctx or containerInfo or img or cropInfo`);
      if (!inited && !privateCall)
        return;
      event.preventDefault();
      if (spaceKeyDown)
        return;
      if ((startMousePoint.x !== void 0 || endMousePoint.x !== void 0) && !event.onTouchMove)
        return;
      if (!isWheeled) {
      ...
      drawTagList(ctx2, tagArr, zoomScale, currentPosition, void 0, origin);
      isWheeled = true;
      hasHoverRectInCanvas = false;
      resetWheelStatus();
    }
    ...
    function cleartMousePoints() {
      if (!inited || !cropInfo)
        return;
      if (!spaceKeyDown) {
        if (tmpCurrentPosition)
          currentPosition = lodash.exports.cloneDeep(tmpCurrentPosition);
        tmpCurrentPosition = void 0;
        console.log("CurrentPosition", currentPosition);
        console.log("cropInfo", cropInfo, transfromBoundingBoxToLtwh(cropInfo, cropScale, zoomScale, currentPosition));
        ...
        if (startMousePoint.x !== void 0 && endMousePoint.x !== void 0) {
          if (props.mode === "crop") {
            cropScale = 1;
            if (tmpCropPositionInfo) {
              cropInfo = getRectInfoByPosition(tmpCropPositionInfo, zoomScale, currentPosition);
              console.log("SET CROP INFO", cropInfo);
              triggerCropInfoChange();
              tmpCropPositionInfo = void 0;
            }
          } else {
    ...
    function getTagList(tagList, _cropInfo, initScale2, imageWH) {
      let list = tagList || tagArr;
      let info = cropInfo || getCropBounding();
      let resultList = list.map((tag) => {
        let newTagInfo = lodash.exports.cloneDeep(tag);
        let _scale = tag.scale === 1 ? initScale2 || scale : 1;
    ...
    function getCropBounding(_cropInfo, _cropScale, initScale2, imageWH) {
      let info = _cropInfo || cropInfo;
      if (!info) {
        throw new Error(`none cropInfo`);
      }
      let _scale = (_cropScale || cropScale) === 1 ? initScale2 || scale : 1;
      let result = {
    ...
    function onMouseMove(e) {
      if (!inited || !ctx2 || !cropInfo)
        return;
      let event = e;
      mouseUpTime = new Date().getTime();
      if (startMousePoint.x === void 0) {
        moveDrawUnshowTagDashRect(ctx2, props.mode, tagArr, zoomScale, currentPosition, origin, event, cropInfo, cropScale, isWheeling, hasHoverRectInCanvas);
        moveDetectCropBorderSetCursor(containerRef.value, event, props.mode, cropInfo, cropScale, zoomScale, currentPosition, origin, isWheeling);
        return;
      }
      endMousePoint = {
        x: event.layerX,
      ...
      if (!spaceKeyDown) {
        let move = () => {
          ...
          if (offsetInfo) {
            tmpCurrentPosition = lodash.exports.cloneDeep(currentPosition);
            if (!tmpCurrentPosition)
              return;
        ...
        if (props.mode === "crop") {
          if (mouseDownOnCropBorderOrVertex) {
            tmpCropPositionInfo = moveResizeCrop(ctx2, startMousePoint, endMousePoint, cropInfo, cropScale, zoomScale, currentPosition, tagArr, mouseDownOnCropBorderOrVertex);
          } else {
            move();
          }
        }
    ...
    function onClick(event) {
      ...
      let positions = transfromBoundingBoxToLtwh(cropInfo, cropScale, zoomScale, currentPosition);
      drawCropRect(ctx2, ...positions);
      console.log("touchPoint", touchPoint, tagArr);
      let { isReDraw, redrawList } = drawTagList(ctx2, tagArr, zoomScale, currentPosition, void 0, void 0, touchPoint);
      if (isReDraw) {
    ...
    function onTouchStart(event) {
      ...
      if (event.touches.length == 2) {
        ...
        let hypotenuse2 = getHypotenuseValue(width, height);
        console.log("TouchStart", event.touches, width, height, hypotenuse2);
    ...
    async function onTouchMove(event) {
      ...
      if (event.touches.length == 2) {
        let { width, height } = getTwoFingerTouchListDistence(touchList);
        let _hypotenuse = getHypotenuseValue(width, height);
        let distance = _hypotenuse - hypotenuse;
        let zoom = -distance;
    ...
    function removeTagItems(removeList) {
      resetWheelStatus(true);
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        onMouseup: withModifiers(onMouseUp, ["stop"]),
        onMousemove: withModifiers(onMouseMove, ["stop"]),
        onMouseout: withModifiers(onMouseOut, ["stop"]),
        onMousewheel: withModifiers(onMouseWheel, ["stop"]),
        onTouchmove: withModifiers(onTouchMove, ["stop", "prevent"]),
        onTouchstart: withModifiers(onTouchStart, ["stop"]),
        onTouchend: withModifiers(onTouchEnd, ["stop"])
      }, [
        ...
        createElementVNode("div", _hoisted_2, [
          createElementVNode("div", _hoisted_3, [
            createElementVNode("div", {
              class: normalizeClass(["circle", { crop: __props.mode === "crop", tag: __props.mode === "tag" }])
            }, null, 2),
            createElementVNode("div", _hoisted_4, toDisplayString(__props.mode === "crop" ? "\u88C1\u526A\u6A21\u5F0F" : "\u6807\u8BB0\u9519\u8BEF\u884C"), 1)
          ]),
          _hoisted_5
        ])
      ], 40, _hoisted_1);
...
var ImgMark = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-7e867318"]]);
export { ImgMark };
